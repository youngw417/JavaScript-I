...
inventory.forEach( cur => {
  if (cur.id === 33){
    console.log(`Car 33 is ${cur.car_year} year old, ${cur.car_make} make ${cur.car_model}`);
  }
})
...
console.log('last car make: ' + inventory[inventory.length -1].car_make + ', model: ' + inventory[inventory.length -1].car_model);
...
oldCars = carYears.filter(cur => cur < 2000);
console.log('number of old cars before year 2000 is ' + oldCars.length + ' cars');

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory. Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
let BMWAndAudi = [];

// BMWAndAudi = inventory.filter(cur => {
//   //cur.car_make ==='BMW' || cur.car_make === 'Audi'
//   cur.car_year > 2000
// })
// console.log(BMWAndAudi);

inventory.forEach(cur => {
  if (cur.car_make === 'BMW' || cur.car_make ==='Audi'){
    BMWAndAudi.push(cur);
  }
})
...
//      return obj
//    }, {})
//const peopleObject = arrayToObject(BMWAndAudi);
//console.log(peopleObject);

//JSON.sringfy(BMWAndAudi);